...
public class SuffixTreeFromArray1 {
    ...
    Map<Integer, List<Node>> SuffixTreeFromSuffixArray(
            int[] suffixArray,
            int[] lcpArray,
            final String text) {
        ...
        Map<Integer, List<Node>> tree1 = new HashMap<Integer, List<Node>>();

        for(Integer i : tree.keySet()){
            tree1.put(i, new ArrayList<Node>() );
            for(Character ch : ACGT){
                if( tree.get(i).children.containsKey(ch) ){
                    tree1.get(i).add( tree.get( tree.get(i).children.get(i) ));
